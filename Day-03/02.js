...
const getIntersectionPoints = ( pointsArr1, pointsArr2 ) => {
    ...
    for ( let steps1 = 1; steps1 < pointsArr1.length; steps1++ ) {
        for (let steps2 = 1; steps2 < pointsArr2.length; steps2++ ) {
            ...
            // Intersection point [x, y] and step for itersection by each wire [ steps_for_x_wire, steps_for_y_wire ]                                      
...
const setWiresArrToIntersection = ( arr, pointsArr1, pointsArr2 ) => {
    ...
    arr.filter( ( val, idx, arr ) => {
        if ( idx % 2 ) {
            ...
            const distanceToIntersection = getDistanceToIntersection( pointsOneToIntersection, pointsTwoToIntersection );
            pointsToIntersections.push( distanceToIntersection );
        }
    } );
    return( pointsToIntersections );
}
